...
QLabel {{ color: {fg_muted}; font-size: 11px; }}
...
QStatusBar {{ background-color: {status_bg}; border-top: 1px solid {border}; color: {status_fg}; font-size: 11px; }}
...
QProgressBar {{ border: 1px solid {border}; background: {bg2}; text-align: center; color: {fg_muted}; font-size: 10px; }}
...
class SchemaPanel(QGroupBox):
    ...
    def _build(self):
        ...
        enum_row.addWidget(self.inp_enum_to); enum_row.addWidget(self.btn_enum)
    ...
    def _on_enum_start(self):
        ...
        self._enum_worker = SchemaEnumWorker(url, id_from, id_to, self._tls_cfg(), proxy_url)
...
class HeadersWidget(QWidget):
    ...
    def _build(self):
        ...
        self.btn_add_row = QPushButton("+"); self.btn_add_row.setFixedWidth(30)
        self.btn_del_row = QPushButton("−"); self.btn_del_row.setFixedWidth(30)
...
class ReaderPanel(QGroupBox):
    ...
    def _on_read(self):
        ...
        tls_cfg = self._cert_panel.get_tls_config().get("broker") if self._cert_panel else None
...
class KafkaProducerWorker(QThread):
    ...
    def __init__(self, broker: str, topic: str,
                 messages: list,           # list[(key: bytes|None, value: bytes)]
                 tls_cfg:  dict | None,
                 headers:  list | None = None):   # list[(str, str)]
        super().__init__()
        self.broker   = broker
        self.topic    = topic
    ...
    def _build_conf(self) -> dict:
        conf = {"bootstrap.servers": self.broker, "acks": "all",
                "retries": 0, "delivery.timeout.ms": 15000}
        if self.tls_cfg:
            conf["security.protocol"] = "SSL"
            ca   = self.tls_cfg.get("ca_cert",    "").strip()
            cert = self.tls_cfg.get("client_cert","").strip()
...
class WriterPanel(QGroupBox):
    ...
    def _on_send(self):
        ...
        if self.chk_raw.isChecked():
            ...
            self._fire_worker(broker, topic, messages, tls_cfg, headers); return
        ...
        self.send_log.append_line(
            ...
            f"avro={'yes' if schema_active else 'no (plain JSON)'}  encryption={enc_mode}", "muted"
        )
        ...
        tls_cfg = self._cert_panel.get_tls_config().get("broker") if self._cert_panel else None
        self.send_log.append_line(
            f"producing to {broker}/{topic}  mTLS={'yes' if tls_cfg else 'no'}", "muted"
        )
        self._fire_worker(broker, topic, messages, tls_cfg, headers)

    def _fire_worker(self, broker, topic, messages, tls_cfg, headers):
        self.btn_send.setEnabled(False)
        self._worker = KafkaProducerWorker(broker, topic, messages, tls_cfg, headers)
...
class KafkaReconWorker(QThread):
    brokers_found = pyqtSignal(list)   # list[str]  "id:host:port"
    topics_found  = pyqtSignal(list)   # list[(topic, partition_count)]
    groups_found  = pyqtSignal(list)   # list[str]
    acls_found    = pyqtSignal(str)    # raw text
    ...
    def __init__(self, broker: str, tls_cfg: dict | None, phases: list[str]):
        ...
        self.broker  = broker
        self.tls_cfg = tls_cfg
        self.phases  = phases   # ["topology", "groups", "acls"]
...
class ReconPanel(QGroupBox):
    ...
    def _run(self, phases: list[str]):
        ...
        self._worker = KafkaReconWorker(broker, tls_cfg, phases)
...
class KafkaOffsetWorker(QThread):
    ...
    def __init__(self, broker: str, group: str, topic: str,
                 reset_to: str, offset: int, tls_cfg: dict | None):
        super().__init__()
        self.broker   = broker
        self.group    = group
        self.topic    = topic
    ...
    def run(self):
        ...
        conf = {"bootstrap.servers": self.broker, "group.id": self.group}
        if self.tls_cfg:
            conf["security.protocol"] = "SSL"
            ca   = self.tls_cfg.get("ca_cert",    "").strip()
            cert = self.tls_cfg.get("client_cert","").strip()
...
class AttackPanel(QGroupBox):
    ...
    def _tls(self) -> dict | None:
        return self._cert_panel.get_tls_config().get("broker") if self._cert_panel else None
    ...
    def _on_acl_read(self):
        ...
        worker = KafkaConsumerWorker(broker, topic, "kafkapt-acl-probe", "latest", 0, 1, self._tls())
    ...
    def _on_ghost_join(self):
        ...
        worker = KafkaConsumerWorker(broker, topic, group, "latest", 0, 5, self._tls())
    ...
    def _on_offset_exec(self):
        ...
        worker = KafkaOffsetWorker(broker, group, topic, reset_to, self.inp_offset.value(), self._tls())
...
class MainWindow(QMainWindow):
    ...
    def _build(self):
        ...
        header_row.addWidget(self.lbl_header); header_row.addStretch(); header_row.addWidget(self.btn_theme)
    ...
    def _toggle_theme(self):
        ...
        QApplication.instance().setStyleSheet(_make_style(self._dark_mode))
        ...
        if self._dark_mode:
            ...
            self.lbl_header.setStyleSheet("color: #2e7d32; letter-spacing: 8px; padding: 4px 0;")
            self.lbl_sub.setStyleSheet("color: #4a7a4a; letter-spacing: 3px; font-size: 11px;")
...
def main():
    ...
    app.setStyleSheet(_make_style(dark=True))
    window = MainWindow()
    window.show()
    sys.exit(app.exec())
