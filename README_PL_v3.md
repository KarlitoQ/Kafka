...
4. [Co nowego w v3.0?](#4-co-nowego-w-v30)
...
KafkaPT v3.0 to **desktopowe narzędzie z interfejsem graficznym** opakowujące typowe zadania pentestów Kafka w wizualny interfejs — bez konieczności zapamiętywania argumentów wiersza poleceń.
...
| **Scan All** | Nieinwazyjny (tylko odczyt) skan bypass ACL + PII/credentiali na wszystkich odkrytych topikach jednym, ograniczonym w tempie przebiegiem |
...
| **Zabezpieczenia** | Dialogi potwierdzenia przed każdą żywą akcją zapisu/injekcji/resetu; ograniczona liczba powtórzeń wysyłki |
| **Integralność dowodów** | Każdy finding zapisywany automatycznie do logu append-only w momencie rejestracji; findingi opatrzone etykietą Target/zlecenia |
...
RECON już teraz odkrywa wszystkie topiki jednym kliknięciem. Wcześniej przetestowanie każdego z nich oznaczało ręczne wpisywanie nazwy topiku w ATTACK lub READ za każdym razem. Scan All kolejkuje istniejące sprawdzenia na wszystkim, co znalazł RECON, więc jedno kliknięcie pokrywa zakres, który wcześniej wymagał N ręcznych przebiegów.

**Co robi:** dla każdego topiku (opcjonalnie odfiltrowanego) podejmuje jedną krótką próbę odczytu. Odebranie jakiejkolwiek wiadomości to ten sam finding **CRITICAL bypass ACL**, co w ATTACK → ACL BYPASS → TEST READ; każda odebrana wiadomość jest też skanowana pod kątem PII/credentiali — dokładnie tak jak w zakładce READ. Działa wyłącznie sekwencyjnie — nigdy równolegle — z konfigurowalnym opóźnieniem i opcjonalnym jitterem między topikami, tym samym wzorcem ograniczania tempa co enumeracja Schema Registry.

**Gdzie:** nowa pod-zakładka **SCAN ALL** w RECON, obok TOPOLOGY / GROUPS / ACLS.

**Celowo wyłączone, nawet jako opcja:** `GHOST JOIN`, `OFFSET RESET`, `TEST WRITE` i `HEADER INJECT`. Są to akcje celujące w grupy lub akcje zapisu/injekcji o realnym potencjale szkody — uruchomienie ich masowo na wszystkich odkrytych topikach lub grupach to zupełnie inne ryzyko niż wykonanie ich pojedynczo, z człowiekiem czytającym każdy dialog potwierdzenia. Pozostają wyłącznie ręczne, po jednym topiku naraz. Skan zapisu/injekcji jako opcja może pojawić się w kolejnej wersji.

**Bezpieczeństwo:** dialog potwierdzenia przed startem pokazuje dokładnie co się wydarzy — liczbę topików, limit wiadomości, opóźnienie — i wprost zaznacza *„Brak zapisów, brak dołączania do grup konsumentów, brak zmian offsetów."* **STOP** anuluje natychmiast topik aktualnie przetwarzany, nie dopiero przed kolejnym.

### Poprzednio w v2.7 — integralność dowodów

Ta wersja dotyczyła tego, żeby nie tracić znalezisk w trakcie zlecenia i nie mieszać dwóch zleceń w jednej sesji.

| Zmiana | Dlaczego to ważne |
|---|---|
| **Automatycznie zapisywany log dowodów.** Każdy finding jest teraz od razu zapisywany do pliku append-only — `kafkapt_sessions/session-<timestamp>.jsonl`, obok skryptu — w momencie rejestracji, a nie dopiero po kliknięciu **EXPORT MARKDOWN**. | Wcześniej findingi i przechwycone wiadomości istniały wyłącznie w pamięci. Awaria lub przypadkowe zamknięcie okna tracono wszystko od ostatniego ręcznego eksportu. Teraz awaria nie traci niczego, co już zostało zarejestrowane — otwórz plik JSONL (jeden finding na linię), aby to odzyskać. Jeśli pliku logu nie da się otworzyć (uprawnienia, dysk tylko do odczytu), KafkaPT przechodzi na tryb tylko-pamięciowy i wyraźnie o tym informuje — w pasku statusu przy starcie oraz etykietą ostrzegawczą na górze FINDINGS — zamiast zawieść po cichu. |
| **Etykieta Target / zlecenia.** Nowe pole **Target:** w pasku nagłówka, obok logo KAFKAPT. | Wpisanie nazwy stempluje ją na każdym findingu rejestrowanym od tego momentu (`Finding.target`), aktualizuje na żywo tytuł okna i pojawia się w widoku szczegółów FINDINGS oraz w nagłówku raportu Markdown. Jeśli kiedykolwiek uruchomisz tę samą, otwartą instancję na drugim środowisku, findingi z obu już się nie mieszają bez żadnego oznaczenia. **Ustaw to przed rozpoczęciem nowego zlecenia.** |

> **Nie ma w v2.7:** ponowne otwarcie poprzedniej sesji — adres brokera, ścieżki certyfikatów, ustawienia proxy — nie jest jeszcze zapisywane/przywracane między uruchomieniami. Zapisywane są wyłącznie findingi. To znana luka do naprawienia w kolejnej wersji.

### Poprzednio w v2.6

Wszystkie poniższe zostały znalezione podczas wewnętrznego przeglądu kodu v2.5 i są poprawkami bezpieczeństwa/poprawności — żadna nowa powierzchnia ataku nie została dodana.

| Poprawka | Dlaczego to miało znaczenie |
|---|---|
| Raport Markdown findingów zawsze wypisywał w nagłówku „KafkaPT v2.0", niezależnie od tego, która wersja go wygenerowała | Raport wygenerowany przez v2.5 błędnie podawał wersję narzędzia czytelnikowi. Teraz odczytuje pojedynczą stałą `APP_VERSION`, używaną wszędzie tam, gdzie pokazywana jest wersja (tytuł okna, pasek statusu, nagłówek raportu). |
| `INJECT` (HEADER INJECT) i `TEST WRITE` (ACL BYPASS) uruchamiały się natychmiast po kliknięciu, bez potwierdzenia — podczas gdy rzadziej klikane `GHOST JOIN` i `EXECUTE OFFSET RESET` już go wymagały | Jedno przypadkowe kliknięcie mogło dostarczyć żywy ładunek injekcji lub nieautoryzowany zapis do docelowego topiku. Oba pokazują teraz ten sam dialog potwierdzenia co pozostałe dwa. |
| **Repeat count** w zakładce SEND pozwalał na do 10 000 nieograniczonych wysyłek jednym kliknięciem | Duże liczby powtórzeń nie mają żadnego ograniczenia tempa i mogły spowodować samoistne przeciążenie lub DoS docelowego brokera. Ograniczono do 500; liczby powyżej 100 wymagają teraz potwierdzenia przed wysłaniem. |
| Krok odkrywania partycji w OFFSET ATTACK tworzył `Consumer` używając **prawdziwego `group.id` docelowej grupy** tylko po to, by odczytać metadane przed resetem | Samo sprawdzenie grupy konsumentów przed resetem jej offsetów mogło już zaburzyć członkostwo w tej grupie. Odkrywanie używa teraz jednorazowego, próbnego `group.id`; sam reset nadal celuje w prawdziwą grupę, bez zmian. |
| Poller Burp Collaboratora miał na stałe wpisane `verify=False` dla swoich żądań HTTPS pollingu, bez możliwości włączenia weryfikacji z powrotem | Adres Poll Server jest konfigurowalny przez operatora i niekoniecznie lokalny, więc ciche pomijanie weryfikacji TLS nie było nigdzie ujawnione w UI. Dodano checkbox **„Verify TLS certificate"** obok Poll Server (domyślnie odznaczony — samopodpisane serwery Collaboratora pozostają typowym przypadkiem — ale teraz widoczny i kontrolowany). |
| `AttackPanel` trzymał w pamięci każdy zakończony wątek workera przez cały czas trwania sesji | Przy długim zleceniu z wieloma powtarzanymi atakami narastało to bez końca. Zakończone workery są teraz czyszczone przed startem każdego kolejnego. |

### Poprzednio w v2.5

Jeśli używałeś KafkaPT v2.0, poniżej znajduje się zestawienie zmian z v2.5.
...
```bash
python3 kafka-pt-v3.py
```

Otworzy się okno. Pasek tytułu pokazuje **KafkaPT v3.0 // Kafka Pentest Toolkit** (lub **// &lt;nazwa Twojego zlecenia&gt;** po jej ustawieniu — patrz niżej).

**Stałe kontrolki w nagłówku (zawsze widoczne):**

| Kontrolka | Opis |
|---|---|
| **Target:** pole (nowe w v2.7) | Nazwa bieżącego zlecenia/klienta, np. `AcmeCorp-2026Q1`. Ustaw to jako pierwsze, przed RECON. Stemplowane na każdym findingu od tego momentu i pokazywane w tytule okna — patrz [§4](#4-co-nowego-w-v30). |
...
```
┌──────────────────────────────────────────────────────────────────────┐
│  KAFKAPT          Target: [AcmeCorp-2026Q1]  Font: [100%▾]  [☀ LIGHT]│
│  Kafka Pentest Toolkit / v3.0                                        │
│──────────────────────────────────────────────────────────────────────│
...
| **Verify TLS certificate** (nowe w v2.6) | Domyślnie odznaczone — żądanie pollingu pomija weryfikację TLS, ponieważ samopodpisany serwer Collaboratora to typowy przypadek. Zaznacz, jeśli Twój Poll Server prezentuje certyfikat, który chcesz zweryfikować wobec systemowego magazynu CA. |
...
#### Pod-zakładka SCAN ALL (v3.0)

**Co to jest:** Przemiata wszystkie topiki z ostatniego pobrania TOPOLOGY jednym sprawdzeniem tylko-do-odczytu na topik, zamiast ręcznego wpisywania każdej nazwy topiku w ATTACK/READ. Najpierw uruchom **FETCH TOPOLOGY** lub **RUN ALL** — ta pod-zakładka pokazuje ile topików jest aktualnie dostępnych.

**Co sprawdza, per topik:** próbuje krótkiego odczytu (do konfigurowalnej liczby wiadomości). Odebranie jakiejkolwiek wiadomości to ten sam finding **CRITICAL** bypass ACL co ATTACK → ACL BYPASS → TEST READ; każda odebrana wiadomość jest też skanowana pod kątem PII/credentiali, tak jak w zakładce READ.

| Pole / Przycisk | Opis |
|---|---|
| **Include (regex)** | Skanowane są tylko topiki pasujące do tego wzorca. Puste = wszystkie topiki. |
| **Exclude (regex)** | Topiki pasujące do tego wzorca są pomijane, nawet jeśli pasowały do Include — użyj tego, by wykluczyć topiki poza zakresem zlecenia z automatycznego skanu. |
| **Max msgs/topic** | Limit wiadomości na topik (1–50, domyślnie 5). |
| **From** | `earliest` (domyślnie) lub `latest` — to samo znaczenie co Start Offset w zakładce READ. |
| **Delay** | Milisekundy oczekiwania między topikami (domyślnie 500ms). |
| **Jitter** | Dodaje ±30% losową wariację do opóźnienia, tak jak tryb stealth enumeracji Schema Registry. |
| **SCAN ALL** | Pokazuje dialog potwierdzenia z dokładnym opisem co się wykona (liczba topików, limit wiadomości, opóźnienie), następnie uruchamia skan. |
| **STOP** | Anuluje natychmiast topik aktualnie przetwarzany, nie dopiero przed kolejnym. |

> **Nie ma tu, nawet jako opcji:** `GHOST JOIN`, `OFFSET RESET`, `TEST WRITE` i `HEADER INJECT`. Pozostają wyłącznie ręczne w zakładce ATTACK, po jednym topiku (lub grupie) naraz, z własnymi dialogami potwierdzenia — uruchomienie akcji zapisu lub celujących w grupy na wszystkim, co znalazł RECON, to zupełnie inne ryzyko niż nieinwazyjny skan Scan All.
...
| **INJECT** | Przed wykonaniem pojawia się dialog potwierdzenia (nowe w v2.6). Wysyła jedną wiadomość ze wszystkimi skonfigurowanymi nagłówkami. Przy powodzeniu dodawany jest finding **HIGH**. |
...
| **TEST WRITE** | Przed wykonaniem pojawia się dialog potwierdzenia (nowe w v2.6). Próba wyprodukowania wiadomości-markera do docelowego topiku. Sukces → finding **CRITICAL**. |
...
> **Uwaga bezpieczeństwa v2.6:** Przed wykonaniem resetu KafkaPT krótko odczytuje listę partycji topiku, aby wyliczyć nowe offsety. Ten krok odkrywania używa teraz jednorazowej, próbnej grupy konsumentów, nigdy grupy będącej celem — wcześniejsze wersje używały do tego kroku (tylko-do-odczytu) `group.id` samej docelowej grupy, co mogło zaburzyć jej członkostwo jeszcze zanim potwierdziłeś reset.
...
| **Repeat count** | Liczba powtórzeń. Ograniczona do 500 (v2.6). Liczby powyżej 100 pokazują dialog potwierdzenia przed wysłaniem, ponieważ duże powtórzenia nie mają wbudowanego ograniczenia tempa i mogą obciążyć lub zakłócić działanie docelowego brokera. |
...
**Automatycznie zapisywane dowody (v2.7):** Etykieta na górze tej zakładki pokazuje, gdzie findingi są automatycznie zapisywane — `kafkapt_sessions/session-<timestamp>.jsonl`, obok skryptu, aktualizowana w momencie rejestracji każdego findingu. To dodatkowy mechanizm, niezależny od ręcznego **EXPORT MARKDOWN** poniżej. Jeśli etykieta zamiast tego pokazuje ostrzeżenie, że logu nie udało się otworzyć, findingi istnieją tylko w pamięci na tę sesję — eksportuj ręcznie i częściej.

**Etykieta Target:** Cokolwiek wpisałeś w pole **Target:** w pasku nagłówka (patrz [§7](#7-uruchomienie-narzędzia)) jest stemplowane na każdym findingu i pokazywane w jego widoku szczegółów oraz w nagłówku eksportu Markdown.
...
Pełne szczegóły wybranego findingu: ważność, tytuł, target (jeśli ustawiony), faza, timestamp, pełny opis ryzyka i surowy dowód.

---

## 17. Typowy przebieg testu penetracyjnego

### Faza 0 — Nazwij zlecenie (pasek nagłówka)

0. Wpisz nazwę w pole **Target:** w pasku nagłówka, np. `AcmeCorp-2026Q1`. Każdy finding od tego momentu jest nią opatrzony, a nazwa pojawia się w tytule okna, więc od razu widać do którego zlecenia należy dane okno. Zrób to przed Fazą 2.
...
### Faza 2b — Scan All (RECON → SCAN ALL) — v3.0

8a. Przejdź do pod-zakładki **SCAN ALL**. Ustaw filtry Include/Exclude jeśli część odkrytych topików jest poza zakresem. Kliknij **SCAN ALL**, potwierdź dialog i pozwól przeskanować wszystkie topiki pod kątem bypass ACL + PII/credentiali, zanim zaczniesz ręcznie celować w pojedyncze topiki w Fazie 3–4 poniżej.
...
15. Kliknij **INJECT** (potwierdź dialog). Monitoruj zakładkę FINDINGS pod kątem callbacków Collaboratora.
...
20. Kliknij **TEST WRITE** (potwierdź dialog). Zanotuj wynik.
...
### Automatyczny log dowodów (v2.7, automatyczny — bez akcji z Twojej strony)

Każdy finding jest zapisywany do `kafkapt_sessions/session-<timestamp>.jsonl` (obok skryptu) w momencie rejestracji, przez cały czas trwania sesji — dzieje się to automatycznie od chwili otwarcia KafkaPT, jeszcze zanim cokolwiek wyeksportujesz ręcznie. Jeden obiekt JSON na linię, wraz z etykietą target. Traktuj to jako siatkę bezpieczeństwa na wypadek awarii sesji lub przypadkowego zamknięcia okna; dwa eksporty poniżej pozostają sposobem na wygenerowanie właściwego raportu.
...
*KafkaPT v3.0 — opracowany dla społeczności badaczy bezpieczeństwa.*
