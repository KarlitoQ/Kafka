...
4. [What's New in v3.0](#4-whats-new-in-v30)
...
KafkaPT v3.0 is a **desktop GUI tool** that wraps common Kafka penetration testing tasks into a visual interface, so you don't have to memorise command-line arguments or write custom scripts.
...
| **Scan All** | Read-only ACL-bypass + PII/credential sweep across every discovered topic in one queued, rate-limited run |
...
| **Safety controls** | Confirmation dialogs before any live write/injection/reset action; capped repeat-send count |
| **Evidence integrity** | Every finding auto-saved to an append-only log as it's recorded; findings stamped with a Target/engagement label |
...
RECON already discovers every topic in one click. Previously, testing each one meant manually retyping the topic name into ATTACK or READ for every single topic. Scan All queues the existing checks across everything RECON found, so one click covers ground that used to take N manual passes.

**What it does:** for every topic (optionally filtered), attempts one short read. Any message received is the same **CRITICAL ACL-bypass finding** as ATTACK → ACL BYPASS → TEST READ; every message received is also run through the same **PII/credential scan** as the READ tab. Runs strictly sequentially — never in parallel — with a configurable delay and optional jitter between topics, same rate-limit pattern already used for Schema Registry enumeration.

**Where:** a new **SCAN ALL** sub-tab inside RECON, alongside TOPOLOGY / GROUPS / ACLS.

**Deliberately excluded, even as an opt-in:** `GHOST JOIN`, `OFFSET RESET`, `TEST WRITE`, and `HEADER INJECT`. These are group-targeted or write/injection actions with real blast radius — batching them across every discovered topic or group is a materially different risk than running them one at a time with a human reading each confirmation dialog. They stay manual-only, one topic at a time. A write/injection sweep may follow as an opt-in in a later release.

**Safety:** a pre-flight confirmation dialog states exactly what will run — topic count, message cap, delay — before anything starts, and explicitly confirms *"No writes, no consumer-group joins, no offset changes."* `STOP` cancels the topic currently in progress immediately, not just before the next one.

### Previously in v2.7 — evidence integrity

This release is about not losing what you found mid-engagement, and not letting two engagements bleed into each other.

| Change | Why it matters |
|---|---|
| **Auto-persisted evidence log.** Every finding is now written immediately to an append-only file — `kafkapt_sessions/session-<timestamp>.jsonl`, next to the script — the moment it's recorded, not only when you click **EXPORT MARKDOWN**. | Previously, findings and captured messages lived only in memory. A crash or an accidental window close lost everything since the last manual export. Now a crash loses nothing already recorded — reopen the JSONL file (one finding per line) to recover it. If the log file can't be opened (permissions, read-only disk), KafkaPT falls back to in-memory-only and says so clearly — in the status bar at startup and as a warning label at the top of FINDINGS — rather than failing silently. |
| **Target / engagement label.** A new **Target:** field in the header bar, next to the KAFKAPT logo. | Typing a name stamps it onto every finding recorded from that point on (`Finding.target`), updates the window title live, and appears in the FINDINGS detail view and the Markdown report header. If you ever run the same open instance against a second environment without restarting it, findings from the two no longer silently interleave with nothing to tell them apart. **Set this before you start a new engagement.** |

> **Not included in v2.7:** reopening a previous session — broker address, certificate paths, proxy settings — is not yet saved/restored between runs. Only findings are persisted. This is a known gap for a future release.

### Previously in v2.6

All of these were found during an internal code review of v2.5 and are safety/correctness fixes — no new attack surface was added.

| Fix | Why it mattered |
|---|---|
| Markdown findings report always printed "KafkaPT v2.0" in its header, regardless of which version generated it | A report generated by v2.5 misreported the tool version to whoever read it. Now reads a single `APP_VERSION` constant, referenced everywhere the version is shown (window title, status bar, report header). |
| `INJECT` (HEADER INJECT) and `TEST WRITE` (ACL BYPASS) fired immediately on click, with no confirmation — while the less frequently used `GHOST JOIN` and `EXECUTE OFFSET RESET` already required one | A single accidental click could deliver a live injection payload or an unauthorized write to a target topic. Both now show the same confirm-before-send dialog as the other two. |
| `SEND` tab's **Repeat count** allowed up to 10,000 unthrottled sends in a single click | Large repeat counts have no rate limiting and could self-inflict load or a DoS against the target broker. Capped to 500; any count above 100 now asks for confirmation before sending. |
| `OFFSET ATTACK`'s partition-discovery step created a `Consumer` using the **target group's real `group.id`** just to read partition metadata before the reset | Merely inspecting a consumer group before resetting its offsets could itself perturb that group's membership. Discovery now uses a disposable probe `group.id`; the actual reset still targets the real group, unchanged. |
| Burp Collaborator poller hardcoded `verify=False` for its HTTPS polling requests, with no way to turn verification back on | The Poll Server address is operator-configurable and not necessarily local, so silently skipping TLS verification wasn't disclosed anywhere in the UI. Added a **"Verify TLS certificate"** checkbox next to Poll Server (unchecked by default — self-signed Collaborator servers remain the common case — but now visible and controllable). |
| `AttackPanel` kept every finished background worker thread in memory for the life of the session | On a long engagement with many repeated attack runs, this accumulated indefinitely. Finished workers are now pruned before each new one starts. |

### Previously in v2.5

If you used KafkaPT v2.0, here is a summary of what v2.5 changed.
...
The window opens. Title bar shows **KafkaPT v3.0 // Kafka Pentest Toolkit** (or **// &lt;your target name&gt;** once you've set one — see below).

**Header controls (always visible):**

| Control | Description |
|---|---|
| **Target:** field (new in v2.7) | Name of the current engagement/client, e.g. `AcmeCorp-2026Q1`. Set this first, before RECON. Stamped on every finding recorded from that point on and shown in the window title — see [§4](#4-whats-new-in-v27). |
...
| **Verify TLS certificate** (new in v2.6) | Unchecked by default — the poll request skips TLS verification, since a self-hosted Collaborator server commonly uses a self-signed certificate. Check this if your Poll Server presents a certificate you want validated against the system CA store. |
...
#### SCAN ALL sub-tab (v3.0)

**What it is:** Sweeps every topic from the last TOPOLOGY fetch with one read-only check each, instead of retyping each topic name into ATTACK/READ by hand. Run **FETCH TOPOLOGY** or **RUN ALL** first — this sub-tab shows how many topics are currently available.

**What it checks, per topic:** attempts a short consume (up to a configurable message count). Any message received is the same **CRITICAL** ACL-bypass finding as ATTACK → ACL BYPASS → TEST READ; every message received is also scanned for PII/credentials, same as the READ tab.

| Field / Button | Description |
|---|---|
| **Include (regex)** | Only topics matching this pattern are scanned. Leave blank to include all. |
| **Exclude (regex)** | Topics matching this pattern are skipped, even if they matched Include — use this to keep out-of-scope topics out of an automated sweep. |
| **Max msgs/topic** | Message cap per topic (1–50, default 5). |
| **From** | `earliest` (default) or `latest` — same meaning as the READ tab's Start Offset. |
| **Delay** | Milliseconds to wait between topics (default 500ms). |
| **Jitter** | Adds ±30% random variation to the delay, same as Schema Registry enumeration's stealth mode. |
| **SCAN ALL** | Shows a confirmation dialog stating exactly what will run (topic count, message cap, delay), then starts the sweep. |
| **STOP** | Cancels the topic currently in progress immediately, not just before the next one starts. |

> **Not included, even as an opt-in:** `GHOST JOIN`, `OFFSET RESET`, `TEST WRITE`, and `HEADER INJECT`. These stay manual-only in the ATTACK tab, one topic (or group) at a time, with their own confirmation dialogs — batching write or group-targeted actions across everything RECON found is a materially different risk than Scan All's read-only sweep.
...
| **INJECT** | A confirmation dialog appears before execution (new in v2.6). Sends one message with all configured headers. A **HIGH** finding is added on successful delivery. |
...
6. Click **INJECT** (confirm the dialog).
...
| **TEST WRITE** | A confirmation dialog appears before execution (new in v2.6). Attempts to produce a marker message to the target topic. Success → **CRITICAL** finding. |
...
> **v2.6 safety note:** Before executing the reset, KafkaPT briefly reads the topic's partition list to compute the new offsets. This discovery step now uses a disposable probe consumer group, never the group you're targeting — earlier versions used the target group's own `group.id` for this read-only step, which could itself perturb that group's membership before you even confirmed the reset.
...
| **Repeat count** | How many times to send. Capped at 500 (v2.6). Counts above 100 show a confirmation dialog before sending, since a large repeat has no built-in rate limiting and could load or disrupt the target broker. |
...
**Auto-saved evidence (v2.7):** A label at the top of this tab shows where findings are being auto-saved — `kafkapt_sessions/session-<timestamp>.jsonl`, next to the script, updated the instant each finding is recorded. This is separate from, and in addition to, the manual **EXPORT MARKDOWN** below. If the label instead shows a warning that the log couldn't be opened, findings exist only in memory for that session — export manually and more often.

**Target label:** Whatever you typed into the **Target:** field in the header bar (see [§7](#7-launching-the-tool)) is stamped on every finding and shown in its detail view and in the Markdown export header.
...
Full details of the selected finding: severity, title, target (if one was set), phase, timestamp, full description explaining the risk, and raw evidence.
...
### Phase 0 — Name the engagement (header bar)

0. Type a name into the **Target:** field in the header bar, e.g. `AcmeCorp-2026Q1`. Every finding from this point on is stamped with it, and it appears in the window title so you can tell at a glance which engagement a running window belongs to. Do this before Phase 2.
...
### Phase 2b — Scan All (RECON → SCAN ALL) — v3.0

8a. Switch to the **SCAN ALL** sub-tab. Set Include/Exclude filters if some discovered topics are out of scope. Click **SCAN ALL**, confirm the dialog, and let it sweep every topic for ACL-bypass + PII/credential findings before you start manually targeting individual topics in Phase 3–4 below.
...
15. Click **INJECT** (confirm the dialog). Monitor FINDINGS for Collaborator callbacks.
...
20. Click **TEST WRITE** (confirm the dialog). Note the result.
...
### Auto-saved evidence log (v2.7, automatic — no action needed)

Every finding is written to `kafkapt_sessions/session-<timestamp>.jsonl` (next to the script) the instant it's recorded, for the entire life of the session — this happens automatically from the moment KafkaPT opens, before you export anything manually. One JSON object per line, including the target label. Treat this as your safety net if the session crashes or the window is closed by accident; the two exports below remain the way to produce a deliverable report.
...
*KafkaPT v3.0 — developed for the security research community.*
