"""
KafkaPT v3.0  —  Kafka Pentest Toolkit
========================================
...
v3.0  (current) — Scan All
----------------------------------------
  • New: RECON → SCAN ALL sub-tab. Sweeps every topic from the last
    topology fetch with ONE read-only check per topic (not per-topic
    manual retyping into ATTACK/READ): any message received is the same
    ACL-bypass finding as ATTACK → ACL BYPASS → TEST READ, and every
    message received is run through the same PII/credential scan as the
    READ tab (scan_pii — hoisted out of ReaderPanel to a module-level
    function so both call sites share one implementation).
  • ScanAllWorker (QThread) drives the sweep strictly sequentially, never
    in parallel, with configurable delay + optional jitter between
    topics (same rate-limit pattern as Schema Registry enumeration). It
    reuses KafkaConsumerWorker.run() directly — called as a plain method,
    not via .start(), so it executes synchronously inside ScanAllWorker's
    own thread — rather than reimplementing the poll/EOF/auth-error
    handling loop a second time.
  • Topic Include/Exclude regex filters, a pre-flight confirmation dialog
    stating exactly what will run (topic count, message cap, delay), a
    progress bar, and STOP (cancels the in-flight topic immediately via
    a shared stop flag, not just between topics).
  • ReconPanel.get_topics() — topics discovered by RECON were previously
    only rendered into a QTreeWidget with no programmatic access; Scan
    All (and any future consumer) can now read them directly.
  • Deliberately excluded, even as an opt-in: GHOST JOIN, OFFSET RESET,
    TEST WRITE, and HEADER INJECT. These are group-targeted or write/
    injection actions with real blast radius — batching them across every
    discovered topic/group is a materially different risk than running
    them one at a time with a human reading each confirmation. They stay
    manual-only. A Tier 2 (opt-in write/injection sweep) may follow in a
    later release.

v2.7  — evidence integrity
----------------------------------------
  • Auto-persisted evidence log — every Finding is now written immediately
    to an append-only JSONL file (kafkapt_sessions/session-<timestamp>.jsonl,
    next to this script) as soon as it's recorded, not just when the
    operator remembers to click EXPORT MARKDOWN in FINDINGS. A crash or an
    accidental window close no longer loses evidence collected up to that
    point. FindingStore.start_log()/_append_log() — never raises; a
    disk/IO failure degrades to in-memory-only with a visible warning
    (status bar + a label in FINDINGS) rather than crashing the session.
  • Target / engagement label — new "Target:" field in the header bar.
    FindingStore.set_target() stamps it onto every Finding.target from
    that point on (Finding gained a `target` field), shown in the window
    title, the FINDINGS detail view, and the Markdown export header.
    Prevents findings from two different engagements silently interleaving
    if the same running instance is pointed at a second environment.
  • Note: session/project save-load (reopening broker+cert config between
    runs) is deliberately out of scope for v2.7 — see README roadmap notes.

v2.6
----------------------------------------
  • Fixed: Markdown findings report always said "KafkaPT v2.0" in its
    header regardless of which version generated it. Now reads APP_VERSION.
  • Fixed: INJECT and TEST WRITE fired immediately with no confirmation,
    while the less-frequently-clicked GHOST JOIN / OFFSET RESET already
    required one. Both now show the same confirm-before-send dialog.
  • Fixed: SEND tab's Repeat count allowed up to 10 000 unthrottled sends
    in one click. Capped to 500; counts above 100 now require confirmation
    to avoid an accidental self-inflicted DoS against the target broker.
  • Fixed: OFFSET ATTACK's partition-discovery step created a Consumer
    using the *target* group's real group.id just to read metadata,
    which could itself perturb that group before the reset even ran.
    Now uses a disposable probe group.id for discovery only; the actual
    reset still targets self.group via AdminClient, unchanged.
  • Fixed: Burp Collaborator poller hardcoded verify=False with no way
    to re-enable TLS verification, even though the poll server address
    is a configurable, potentially non-local URL. Added a "Verify TLS
    certificate" checkbox next to Poll Server (unchecked by default —
    self-signed Collaborator servers remain the common case — but now
    visible and operator-controlled instead of silent).
  • Fixed: AttackPanel accumulated every finished QThread worker in
    self._workers for the life of the session (INJECT / ACL / GHOST /
    OFFSET all appended, nothing ever removed). Finished workers are now
    pruned before each new one starts.

v2.5  (Sprint 0 + 1 + 2 + 3)
----------------------------------------

UI / UX Fixes  (Sprint 0)
  • Fixed + / − buttons in HeadersWidget — were invisible due to global
    QPushButton padding (6px 16px) crushing the 30px-wide buttons.
    Replaced with QPushButton#btn_icon CSS class (zero padding, fixed size).
  • Added font scaling for HiDPI / 4K displays: combo 100 % / 125 % / 150 % / 175 %
    in the header bar. _make_style(dark, scale) scales all CSS font-size values.
    _apply_scale() updates QFont instances on all QTextEdit children via
    findChildren() — no manual widget tracking needed.
  • Enabled Qt.HighDpiScaleFactorRoundingPolicy.PassThrough in main().
  • Layout restructured: single QTabWidget with CONFIG as the first tab,
    eliminating the cramped 340 px top config panel. Full tab order:
    ⚙ CONFIG | RECON | ATTACK | CONNECT | READ | SEND | RANDOMIZE | FINDINGS

Security / Pentest Fixes  (Sprint 1)
  • FindingStore deduplication — MD5 hash of (severity|title|evidence[:60])
    prevents duplicate findings from repeated injection attempts.
  • Schema Registry enumeration rate control — configurable inter-request
    delay (0–10 000 ms) with optional ±30 % random jitter (stealth mode).
    Automatic exponential back-off on HTTP 429 (Retry-After respected).
  • Threading cleanup — replaced QThread.terminate() in ReconPanel with a
    clean _stop_flag event checked between recon phases, preventing resource
    leaks and broker connection orphans.
  • SASL authentication support (PLAIN, SCRAM-SHA-256, SCRAM-SHA-512):
    - New SASL section in CertPanel (mechanism, username, password).
    - Module-level _build_kafka_conf(broker, tls_cfg, sasl_cfg, extra) helper
      automatically selects SSL / SASL_PLAINTEXT / SASL_SSL protocol.
    - All four Kafka workers updated: KafkaConsumerWorker, KafkaProducerWorker,
      KafkaReconWorker, KafkaOffsetWorker.
    - All panels pass sasl_cfg to workers alongside tls_cfg.

Sprint 3 — New Modules
  • CollaboratorPoller — polls a self-hosted Burp Collaborator server
    (POST /burpresults?biid=...) at a configurable interval (5–300 s).
    Each DNS / HTTP / SMTP callback is automatically recorded as a CRITICAL
    finding with full evidence (type, clientIp, interactionId, rawRequest).
    Integrated into ProxyPanel; no external services used.
  • KafkaConnectWorker + ConnectPanel (new CONNECT ops tab) — enumerates
    the Kafka Connect REST API (port 8083):
      GET /connectors              list connector names
      GET /connectors/{n}/config  full config with credential scanning
      GET /connectors/{n}/status  running state
      GET /connector-plugins      installed plugin classes
    Unauthenticated access → MEDIUM finding.
    Credential keywords or JDBC URLs with embedded passwords → HIGH finding.
    Config view annotates sensitive keys inline.

────────────────────────────────────────────────────────────

v2.0  (original release — Sprint 1 + 2 baseline)
--------------------------------------------------

New modules
  ProxyPanel          HTTP proxy (Burp Suite) + Collaborator URL substitution
  HeadersWidget       Reusable header table with injection payload presets
  FindingStore        Thread-safe singleton finding registry
  FindingsPanel       Findings viewer with severity filter + Markdown export
  SchemaEnumWorker    Schema Registry ID brute-force enumeration
  KafkaReconWorker    Broker metadata / consumer group / ACL enumeration
  ReconPanel          Topology / Groups / ACLs viewer
  AttackPanel         Header injection, ACL bypass, consumer offset attack
  KafkaOffsetWorker   Consumer group offset reset (background thread)

Changes to existing modules
  SchemaFetchWorker   Added proxy_url support (routes through Burp)
  SchemaPanel         Added schema ID range enumeration UI
  KafkaConsumerWorker Message headers captured in message_received signal
  KafkaProducerWorker Added headers parameter (passed to producer.produce)
  ReaderPanel         Headers display, NDJSON export, PII/credential scanner
  WriterPanel         HeadersWidget with injection presets
  MainWindow          Config tabs + pentest ops tabs layout

────────────────────────────────────────────────────────────

Module inventory  (26 classes, 3 535 lines)
  Background workers  SchemaFetchWorker, SchemaEnumWorker, KafkaConsumerWorker,
                      KafkaProducerWorker, KafkaReconWorker, KafkaOffsetWorker,
                      CollaboratorPoller, KafkaConnectWorker
  Config panels       SchemaPanel, CertPanel, EncryptionPanel, ProxyPanel
  Ops panels          ReconPanel, AttackPanel, ConnectPanel, ReaderPanel,
                      WriterPanel, RandomizerPanel, FindingsPanel
  Shared widgets      HeadersWidget, KafkaBrokerBar, LogPane
  Data / logic        Finding, FindingStore, AvroRandomizer
...
# Keywords that indicate sensitive values in Kafka Connect connector configs
CRED_KEYWORDS: frozenset[str] = frozenset({
    "password", "passwd", "secret", "api_key", "apikey",
    "access_key", "access.key", "secret_key", "secret.key",
    "private_key", "private.key", "token", "credential", "credentials",
    "db.password", "connection.password", "database.password",
    "aws.secret", "s3.secret.key", "s3.credentials.provider.secret",
})
...
class Finding:
    ...
    target:         str = ""   # engagement/target label — stamped by FindingStore.add()


class FindingStore:
    """Thread-safe singleton — centralny rejestr findingów.

    v2.7: owns the current engagement/target label (stamped onto every
    Finding as it's added, so findings from two different targets can't
    silently interleave in one running session) and an append-only JSONL
    log that every finding is written to immediately, not just on manual
    Markdown export — a crash or accidental window close no longer loses
    evidence collected before that point.
    """
    ...
    # ── Target / engagement label ─────────────────────────────

    def set_target(self, name: str) -> None:
        self._target = name.strip()

    def get_target(self) -> str:
        return self._target

    # ── Auto-persisted evidence log (v2.7) ────────────────────

    def start_log(self, path: str) -> bool:
        """Open (or reopen) the append-only JSONL evidence log. Returns
        True on success. Never raises — a logging failure must not take
        down a live pentest session."""
        with self._log_lock:
            if self._log_fh:
    ...
    def _append_log(self, finding: Finding) -> None:
        if not self._log_fh:
            return
        try:
            with self._log_lock:
                self._log_fh.write(json.dumps(dataclasses.asdict(finding), ensure_ascii=False) + "\n")
                self._log_fh.flush()
        except Exception:
            pass   # a disk/IO error here must never crash the session

    def add(self, finding: Finding) -> None:
        finding.target = self._target
    ...
    def clear(self) -> None:
        ...
        # Note: CLEAR ALL intentionally does not touch the on-disk log —
        # the log is the durable record; clearing only affects the live
        # in-memory/GUI view. See FindingsPanel._on_clear.
    ...
    def export_markdown(self) -> str:
        ...
        target_line = f"**Target:** {self._target}\n" if self._target else ""
        lines = [
            f"# KafkaPT v{APP_VERSION} — Pentest Findings",
            f"\n_Generated: {ts_now}_\n",
            target_line,
...
def scan_pii(decoded: str, topic: str, partition: int, offset: int) -> None:
    """Regex sweep for credit-card numbers, email addresses, and credential
    keywords in a decoded message body; adds a HIGH finding on any hit.

    v3.0 note: hoisted out of ReaderPanel (where it started as a bound
    method that never touched `self`) so the upcoming Scan All worker can
    call the exact same check per-topic instead of re-implementing it.
    """
    hits = []
...
class ProxyPanel(QGroupBox):
    ...
    def _build(self):
        ...
        self.inp_proxy = QLineEdit(); self.inp_proxy.setText("http://127.0.0.1:8080")
        self.inp_proxy.setEnabled(False)
        self.btn_test = QPushButton("TEST"); self.btn_test.setFixedWidth(60)
        self.btn_test.setEnabled(False); self.btn_test.clicked.connect(self._on_test)
        grid.addWidget(lbl_proxy, 1, 0); grid.addWidget(self.inp_proxy, 1, 1)
        grid.addWidget(self.btn_test, 1, 2)

        scope_row = QHBoxLayout()
        self.chk_scope_sr = QCheckBox("Schema Registry")
        self.chk_scope_sr.setChecked(True); self.chk_scope_sr.setEnabled(False)
        scope_row.addWidget(QLabel("Route:")); scope_row.addWidget(self.chk_scope_sr)
        scope_row.addStretch()
        grid.addLayout(scope_row, 2, 0, 1, 3)

        sep1 = QFrame(); sep1.setFrameShape(QFrame.Shape.HLine)
        grid.addWidget(sep1, 3, 0, 1, 3)

        # ── Collaborator payload URL (COLLAB placeholder) ────
        lbl_collab = QLabel("Collaborator URL"); lbl_collab.setObjectName("lbl_key")
        self.inp_collab = QLineEdit()
        self.inp_collab.setPlaceholderText("abc123.collab.internal  (without http://)")
        self.inp_collab.setToolTip("Domain substituted for COLLAB in injection presets.\nVictim callbacks land here.")
        grid.addWidget(lbl_collab, 4, 0); grid.addWidget(self.inp_collab, 4, 1, 1, 2)

        self.lbl_status = QLabel("proxy disabled")
        self.lbl_status.setObjectName("lbl_status_inf")
        grid.addWidget(self.lbl_status, 5, 0, 1, 3)

        # ── Collaborator Polling section ← Sprint 3 ──────────
        sep2 = QFrame(); sep2.setFrameShape(QFrame.Shape.HLine)
        grid.addWidget(sep2, 6, 0, 1, 3)

        lbl_poll_hdr = QLabel("COLLABORATOR POLLING")
        lbl_poll_hdr.setObjectName("lbl_key")
        grid.addWidget(lbl_poll_hdr, 7, 0, 1, 3)

        lbl_srv = QLabel("Poll Server"); lbl_srv.setObjectName("lbl_key")
        self.inp_poll_server = QLineEdit()
        self.inp_poll_server.setPlaceholderText("http://collab.internal:9090")
        self.inp_poll_server.setToolTip(
            "Internal Burp Collaborator server address used for polling.\n"
            "Different from the payload domain above — this is where KafkaPT\n"
            "sends its POST /burpresults?biid=... requests."
        )
        grid.addWidget(lbl_srv, 8, 0); grid.addWidget(self.inp_poll_server, 8, 1, 1, 2)

        lbl_biid = QLabel("BIID"); lbl_biid.setObjectName("lbl_key")
        self.inp_biid = QLineEdit()
        self.inp_biid.setPlaceholderText("abc123  (polling key from Burp Collaborator tab)")
        self.inp_biid.setToolTip(
            "Burp Collaborator BIID (Burp Interaction ID).\n"
            "Get this from Burp Pro: Collaborator → Copy to clipboard,\n"
            "then extract the subdomain prefix before your Collaborator domain."
        )
        grid.addWidget(lbl_biid, 9, 0); grid.addWidget(self.inp_biid, 9, 1)

        self.chk_poll_verify = QCheckBox("Verify TLS certificate")
        self.chk_poll_verify.setChecked(False)
        self.chk_poll_verify.setToolTip(
            "Unchecked (default): TLS verification is skipped when polling the\n"
            "Collaborator server — the common case for a self-hosted server with\n"
            "a self-signed certificate. Check this if your Poll Server presents a\n"
            "certificate you want validated against the system CA store."
        )
        grid.addWidget(self.chk_poll_verify, 9, 2)

        poll_ctrl = QHBoxLayout()
        lbl_iv = QLabel("Interval"); lbl_iv.setObjectName("lbl_key")
        self.inp_poll_interval = QSpinBox()
        self.inp_poll_interval.setRange(5, 300); self.inp_poll_interval.setValue(30)
        self.inp_poll_interval.setSuffix(" s"); self.inp_poll_interval.setFixedWidth(76)
        self.btn_poll_start = QPushButton("START POLLING")
        self.btn_poll_start.setObjectName("btn_accent")
        self.btn_poll_stop  = QPushButton("STOP")
        self.btn_poll_stop.setObjectName("btn_danger")
        self.btn_poll_stop.setEnabled(False)
        poll_ctrl.addWidget(lbl_iv); poll_ctrl.addWidget(self.inp_poll_interval)
        poll_ctrl.addSpacing(8)
        poll_ctrl.addWidget(self.btn_poll_start); poll_ctrl.addWidget(self.btn_poll_stop)
        poll_ctrl.addStretch()
        grid.addLayout(poll_ctrl, 10, 0, 1, 3)

        self.lbl_poll_status = QLabel("polling inactive")
        self.lbl_poll_status.setObjectName("lbl_status_inf")
        grid.addWidget(self.lbl_poll_status, 11, 0, 1, 3)

        lbl_hint = QLabel(
            "Callbacks appear automatically in FINDINGS tab as CRITICAL findings."
        )
        lbl_hint.setObjectName("lbl_status_inf"); lbl_hint.setWordWrap(True)
        grid.addWidget(lbl_hint, 12, 0, 1, 3)

        self.btn_poll_start.clicked.connect(self._on_start_polling)
        self.btn_poll_stop.clicked.connect(self._on_stop_polling)
    ...
    def _on_start_polling(self):
        server = self.inp_poll_server.text().strip()
        biid   = self.inp_biid.text().strip()
        if not server:
            self._set_poll_status("! Poll Server URL is required", "err"); return
        if not biid:
            self._set_poll_status("! BIID (polling key) is required", "err"); return
        if self._poller and self._poller.isRunning():
            return

        interval = self.inp_poll_interval.value()
        self._poller = CollaboratorPoller(server, biid, interval,
                                          verify_tls=self.chk_poll_verify.isChecked())
        self._poller.interaction_received.connect(self._on_interaction)
        self._poller.status.connect(lambda m: self._set_poll_status(m, "inf"))
        self._poller.error.connect(lambda m: self._set_poll_status(f"! {m}", "err"))
        self._poller.start()

        self.btn_poll_start.setEnabled(False)
        self.btn_poll_stop.setEnabled(True)
        self._set_poll_status(f"polling {server}  every {interval}s …", "warn")

    def _on_stop_polling(self):
        if self._poller:
            self._poller.stop()
        self.btn_poll_start.setEnabled(True)
        self.btn_poll_stop.setEnabled(False)
        self._set_poll_status("polling stopped", "inf")

    def _on_interaction(self, interaction: dict):
        itype     = interaction.get("type", "unknown").upper()
        iid       = interaction.get("interactionId", interaction.get("interaction_id", "?"))
        client_ip = interaction.get("client", interaction.get("clientIp", "?"))
        ts        = interaction.get("time", interaction.get("timestamp", ""))
        raw_req   = interaction.get("rawRequest", interaction.get("rawRequestDecoded", ""))

        evidence = (
            f"type={itype}  interactionId={iid}  clientIp={client_ip}\n"
            f"timestamp: {ts}\n"
            + (f"rawRequest (truncated):\n{raw_req[:400]}" if raw_req else "")
        )
        FindingStore.get().add(Finding(
            severity="CRITICAL",
            title=f"Burp Collaborator callback: {itype} from {client_ip}",
            description=(
                f"A {itype} interaction was received by Burp Collaborator "
                f"(interactionId={iid}, clientIp={client_ip}). "
                "This confirms a server-side request was triggered by an injection payload — "
                "the downstream Kafka consumer processed the injected value and initiated an "
                "outbound connection, proving exploitability."
            ),
            evidence=evidence,
            phase="injection",
        ))
        ...
        self._set_poll_status(
            f"[{now}] callback received: {itype} from {client_ip}  id={iid}", "warn"
        )

    def _set_poll_status(self, text: str, kind: str = "inf"):
        obj_map = {"inf": "lbl_status_inf", "warn": "lbl_status_warn",
                   "ok": "lbl_status_ok", "err": "lbl_status_err"}
        self.lbl_poll_status.setText(text)
        self.lbl_poll_status.setObjectName(obj_map.get(kind, "lbl_status_inf"))
        self.lbl_poll_status.style().unpolish(self.lbl_poll_status)
        self.lbl_poll_status.style().polish(self.lbl_poll_status)


# ─────────────────────────────────────────────────────────
#  CollaboratorPoller — self-hosted Burp Collaborator  [Sprint 3]
# ─────────────────────────────────────────────────────────

class CollaboratorPoller(QThread):
    """
    Polls a self-hosted Burp Collaborator server for new interactions.

    Protocol:  POST {server_url}/burpresults?biid={biid}
    Response:  {"responses": [{type, interactionId, client, time, rawRequest, ...}]}

    Runs until stop() is called.  Results are emitted via interaction_received.
    """
    interaction_received = pyqtSignal(dict)   # one dict per interaction
    status               = pyqtSignal(str)
    error                = pyqtSignal(str)

    def __init__(self, server_url: str, biid: str, interval_s: int = 30,
                 verify_tls: bool = False):
        super().__init__()
        self.server_url  = server_url.rstrip("/")
        self.biid        = biid.strip()
    ...
    def run(self):
        poll_url = f"{self.server_url}/burpresults?biid={self.biid}"
        self.status.emit(f"started — polling {poll_url}  interval={self.interval_s}s")
        ...
        while not self._stop.is_set():
            try:
                r = requests.post(
                    poll_url,
                    timeout=10,
                    verify=self.verify_tls,   # operator-controlled — see ProxyPanel checkbox
                    headers={"Content-Type": "application/json"},
                )
                if r.status_code == 401:
                    self.error.emit(
                        "HTTP 401 — check BIID and Collaborator server access permissions"
                    )
                    break
                if r.status_code == 404:
                    self.error.emit(
                        "HTTP 404 — Collaborator endpoint not found  "
                ...
                if r.ok:
                    try:
                        ...
                        for interaction in interactions:
                            self.interaction_received.emit(dict(interaction))
                        if interactions:
                            self.status.emit(
                                f"{len(interactions)} new interaction(s) received"
                            )
                    except ValueError as exc:
                        self.error.emit(f"JSON parse error: {exc}  raw={r.text[:120]}")
                else:
                ...
                self.error.emit("poll request timed out (10 s)")
            except Exception as exc:
                self.error.emit(f"unexpected poll error: {exc}")

            # Sleep in 0.5s ticks so stop() takes effect quickly
...
class ReaderPanel(QGroupBox):
    ...
    def _on_message(self, topic, partition, offset, ts_type, ts_ms, decoded, raw, headers):
        ...
        scan_pii(decoded, topic, partition, offset)
...
class WriterPanel(QGroupBox):
    ...
    def _build(self):
        ...
        self.inp_repeat_n = QSpinBox(); self.inp_repeat_n.setRange(1, 500)
        self.inp_repeat_n.setToolTip(
            "Capped at 500 to limit accidental self-inflicted load against the\n"
            "target broker. Counts above 100 ask for confirmation before sending."
        )
    ...
    def _on_send(self):
        ...
        if repeat > 100:
            reply = QMessageBox.warning(
                self, "Repeat Send — Confirm",
                f"About to send this message {repeat} times to '{topic}'.\n\n"
                "A large repeat count can itself degrade or disrupt the target broker.\n\n"
...
# ─────────────────────────────────────────────────────────
#  ScanAllWorker — v3.0 Tier 1 (read-only) Scan All  [NEW]
# ─────────────────────────────────────────────────────────

class ScanAllWorker(QThread):
    """
    Sweeps every topic RECON discovered with ONE read-only check per topic:
    a short consume attempt. Any message received is itself the ACL-bypass
    finding (same as ATTACK -> ACL BYPASS -> TEST READ), and every message
    received is run through scan_pii() (same check as the READ tab).

    Deliberately does NOT reimplement the poll/EOF/error-handling loop —
    it reuses KafkaConsumerWorker.run() directly, called as a plain method
    (not via .start()) so it executes synchronously inside this worker's
    own thread. This is the same class the manual READ/ATTACK tabs use,
    so a future fix to that polling logic doesn't need a second fix here.

    Runs topics strictly sequentially (never in parallel) with a
    configurable delay + optional jitter between them — same rate-limit
    pattern as Schema Registry enumeration — so a broad sweep never opens
    a burst of concurrent connections against the target broker.

    Excludes GHOST JOIN / OFFSET RESET and any write/injection check by
    design — see README v3.0 notes. Those stay manual-only, one topic at
    a time, with their own explicit confirmation dialogs.
    """
    topic_started = pyqtSignal(str, int, int)   # topic, index (1-based), total
    topic_done    = pyqtSignal(str, int)         # topic, messages received
    status        = pyqtSignal(str)
    error         = pyqtSignal(str)
    progress      = pyqtSignal(int, int)         # topics done, total
    done          = pyqtSignal(int, int)          # topics scanned, topics that yielded data

    def __init__(self, broker: str, topics: list[str],
                 tls_cfg: dict | None, sasl_cfg: dict | None = None,
                 max_messages: int = 5, offset_mode: str = "earliest",
                 delay_ms: int = 500, jitter: bool = True):
        super().__init__()
        self.broker       = broker
        ...
        self.offset_mode   = offset_mode if offset_mode in ("earliest", "latest") else "earliest"
    ...
    def run(self):
        ...
        for i, topic in enumerate(self.topics, 1):
            if self._stop_flag.is_set():
                self.status.emit("Scan All stopped by operator"); break

            self.topic_started.emit(topic, i, total)

            received: list[tuple] = []
            probe = KafkaConsumerWorker(
                self.broker, topic, "kafkapt-scanall-probe",
                self.offset_mode, 0, self.max_messages,
                self.tls_cfg, sasl_cfg=self.sasl_cfg,
            )
            probe.message_received.connect(lambda *a, buf=received: buf.append(a))
            probe.consumer_error.connect(
                lambda m, t=topic: self.status.emit(f"[{t}] {m}")
            )
            probe._stop_flag = self._stop_flag   # shared — STOP cancels the in-flight topic too
            probe.run()                          # synchronous — reuses the exact poll/EOF/error logic

            if received:
                # ACL-bypass finding: added once per topic; FindingStore's own
                # dedup also protects against this firing twice for the same
                # topic across separate Scan All runs, or against a manual
                # TEST READ finding already on record for it.
                FindingStore.get().add(Finding(
                    severity="CRITICAL",
                ...
                for (t_, partition_, offset_, _ts_type, _ts_ms, decoded_, _raw, _headers) in received:
                    scan_pii(decoded_, t_, partition_, offset_)
                yielded += 1

            scanned += 1
            self.topic_done.emit(topic, len(received))
            self.progress.emit(i, total)

            if i < total and not self._stop_flag.is_set() and self.delay_ms > 0:
                delay = self.delay_ms
                if self.jitter:
                    delay = int(delay * random.uniform(0.7, 1.3))
                self._stop_flag.wait(delay / 1000)   # sleep that STOP can interrupt early

        self.status.emit(
            f"Scan All complete — {scanned}/{total} topic(s) scanned, {yielded} yielded data"
        )
        self.done.emit(scanned, yielded)
...
class ReconPanel(QGroupBox):
    def __init__(self):
        ...
        self._cert_panel  = None
        self._worker      = None
        self._scan_worker = None   # ScanAllWorker — separate from the topology/groups/acls worker above
        self._topics: list[tuple[str, int]] = []   # (topic, partition_count) — last RUN ALL/FETCH TOPOLOGY result
        self._build()

    def set_cert_panel(self, p): self._cert_panel = p

    def get_topics(self) -> list[tuple[str, int]]:
        """Topics discovered by the most recent topology fetch, as
        (topic, partition_count) pairs. Used by Scan All (v3.0) to queue
        checks across everything RECON found, instead of re-typing each
        topic name by hand into ATTACK/READ one at a time."""
    ...
    def _build(self):
        ...
        # Scan All  [v3.0, NEW]
        scan_w = QWidget(); scan_v = QVBoxLayout(scan_w); scan_v.setContentsMargins(6, 6, 6, 6)
        scan_v.setSpacing(8)

        lbl_scan_intro = QLabel(
            "Read-only sweep: attempts a short consume on every topic below (from the last "
            "FETCH TOPOLOGY / RUN ALL). Any message received is an ACL-bypass finding; every "
            "message received is also scanned for PII/credentials — same checks as ATTACK → "
            "ACL BYPASS → TEST READ and the READ tab, run once per topic instead of by hand. "
            "Does not touch consumer groups and never writes — GHOST JOIN, OFFSET RESET, "
            "TEST WRITE, and HEADER INJECT stay manual, one topic at a time."
        )
        lbl_scan_intro.setWordWrap(True); lbl_scan_intro.setObjectName("lbl_status_inf")
        scan_v.addWidget(lbl_scan_intro)

        self.lbl_scan_avail = QLabel("0 topic(s) available — run FETCH TOPOLOGY or RUN ALL first")
        self.lbl_scan_avail.setObjectName("lbl_status_warn")
        scan_v.addWidget(self.lbl_scan_avail)

        filt_grid = QGridLayout(); filt_grid.setSpacing(6)
        lbl_inc = QLabel("Include (regex)"); lbl_inc.setObjectName("lbl_key")
        self.inp_scan_include = QLineEdit()
        self.inp_scan_include.setPlaceholderText("blank = all topics")
        lbl_exc = QLabel("Exclude (regex)"); lbl_exc.setObjectName("lbl_key")
        self.inp_scan_exclude = QLineEdit()
        self.inp_scan_exclude.setPlaceholderText("e.g. ^test- | out-of-scope topics")
        filt_grid.addWidget(lbl_inc, 0, 0); filt_grid.addWidget(self.inp_scan_include, 0, 1)
        filt_grid.addWidget(lbl_exc, 1, 0); filt_grid.addWidget(self.inp_scan_exclude, 1, 1)
        scan_v.addLayout(filt_grid)

        rate_row = QHBoxLayout()
        lbl_max = QLabel("Max msgs/topic"); lbl_max.setObjectName("lbl_key")
        self.inp_scan_max = QSpinBox(); self.inp_scan_max.setRange(1, 50); self.inp_scan_max.setValue(5)
        lbl_off = QLabel("From"); lbl_off.setObjectName("lbl_key")
        self.cmb_scan_offset = QComboBox(); self.cmb_scan_offset.addItems(["earliest", "latest"])
        lbl_delay = QLabel("Delay"); lbl_delay.setObjectName("lbl_key")
        self.inp_scan_delay = QSpinBox(); self.inp_scan_delay.setRange(0, 10000)
        self.inp_scan_delay.setValue(500); self.inp_scan_delay.setSuffix(" ms")
        self.chk_scan_jitter = QCheckBox("Jitter"); self.chk_scan_jitter.setChecked(True)
        rate_row.addWidget(lbl_max); rate_row.addWidget(self.inp_scan_max); rate_row.addSpacing(10)
        rate_row.addWidget(lbl_off); rate_row.addWidget(self.cmb_scan_offset); rate_row.addSpacing(10)
        rate_row.addWidget(lbl_delay); rate_row.addWidget(self.inp_scan_delay)
        rate_row.addWidget(self.chk_scan_jitter); rate_row.addStretch()
        scan_v.addLayout(rate_row)

        scan_btn_row = QHBoxLayout()
        self.btn_scan_all  = QPushButton("SCAN ALL"); self.btn_scan_all.setObjectName("btn_accent")
        self.btn_scan_stop = QPushButton("STOP"); self.btn_scan_stop.setObjectName("btn_danger")
        self.btn_scan_stop.setEnabled(False)
        self.scan_progress = QProgressBar(); self.scan_progress.setMaximumHeight(12)
        scan_btn_row.addWidget(self.btn_scan_all); scan_btn_row.addWidget(self.btn_scan_stop)
        scan_btn_row.addWidget(self.scan_progress, 1)
        scan_v.addLayout(scan_btn_row)

        self.scan_log = LogPane("// Scan All output will appear here")
        scan_v.addWidget(self.scan_log, 1)

        self.btn_scan_all.clicked.connect(self._on_scan_all)
        self.btn_scan_stop.clicked.connect(self._on_scan_stop)

        recon_tabs.addTab(scan_w, "SCAN ALL")
        recon_tabs.currentChanged.connect(self._on_recon_tab_changed)
    ...
    def _on_topics(self, topics: list):
        ...
        self._refresh_scan_avail_label()
        if topics:
            FindingStore.get().add(Finding(
                severity="INFO",
    ...
    def _on_recon_tab_changed(self, _idx: int):
        self._refresh_scan_avail_label()

    def _refresh_scan_avail_label(self):
        n = len(self._topics)
        if n == 0:
            self.lbl_scan_avail.setText("0 topic(s) available — run FETCH TOPOLOGY or RUN ALL first")
            self.lbl_scan_avail.setObjectName("lbl_status_warn")
        else:
            self.lbl_scan_avail.setText(f"{n} topic(s) available from the last topology fetch")
            self.lbl_scan_avail.setObjectName("lbl_status_inf")
        self.lbl_scan_avail.style().unpolish(self.lbl_scan_avail)
        self.lbl_scan_avail.style().polish(self.lbl_scan_avail)

    def _filtered_scan_topics(self) -> list[str] | None:
        """Applies the Include/Exclude regex fields to self._topics. Returns
        None (instead of []) on a bad regex, so the caller can distinguish
        "typo in your pattern" from "filters legitimately matched nothing"."""
        include = self.inp_scan_include.text().strip()
        exclude = self.inp_scan_exclude.text().strip()
        try:
            inc_re = re.compile(include) if include else None
            exc_re = re.compile(exclude) if exclude else None
        except re.error as exc:
            self.scan_log.append_line(f"! invalid regex: {exc}", "error")
            return None
        names = [t for t, _p in self._topics]
        if inc_re: names = [t for t in names if inc_re.search(t)]
        if exc_re: names = [t for t in names if not exc_re.search(t)]
        return names

    def _on_scan_all(self):
        broker = self.inp_broker.text().strip()
        if not broker:
            self.scan_log.append_line("! broker address required", "error"); return
        if self._scan_worker and self._scan_worker.isRunning():
            self.scan_log.append_line("! Scan All already running", "warn"); return
        if not self._topics:
            self.scan_log.append_line(
                "! no topics available — run FETCH TOPOLOGY or RUN ALL first", "error"
            ); return

        topics = self._filtered_scan_topics()
        if topics is None:
            return   # bad regex — already logged
        if not topics:
            self.scan_log.append_line(
                "! no topics match the Include/Exclude filters", "warn"
            ); return

        max_msgs    = self.inp_scan_max.value()
        offset_mode = self.cmb_scan_offset.currentText()
        delay_ms    = self.inp_scan_delay.value()
        jitter      = self.chk_scan_jitter.isChecked()

        reply = QMessageBox.warning(
            self, "Scan All — Confirm",
            f"Read-only sweep of {len(topics)} topic(s) on {broker}:\n\n"
            f"  • up to {max_msgs} message(s) per topic, from '{offset_mode}'\n"
            f"  • {len(topics)} sequential read attempt(s), ~{delay_ms} ms apart\n"
            f"  • any message received → CRITICAL ACL-bypass finding\n"
            f"  • every message received → scanned for PII/credentials\n\n"
            "No writes, no consumer-group joins, no offset changes.\n\n"
            "Proceed only with explicit authorisation.",
            QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel,
        )
        if reply != QMessageBox.StandardButton.Ok: return
        ...
        self.scan_log.clear_log()
        self.scan_log.append_line(f"// Scan All starting — {len(topics)} topic(s)", "warn")
        self.scan_progress.setMinimum(0); self.scan_progress.setMaximum(len(topics)); self.scan_progress.setValue(0)
        self.btn_scan_all.setEnabled(False); self.btn_scan_stop.setEnabled(True)

        self._scan_worker = ScanAllWorker(
            broker, topics, tls_cfg, sasl_cfg=sasl_cfg,
            max_messages=max_msgs, offset_mode=offset_mode,
            delay_ms=delay_ms, jitter=jitter,
        )
        self._scan_worker.topic_started.connect(self._on_scan_topic_started)
        self._scan_worker.topic_done.connect(self._on_scan_topic_done)
        self._scan_worker.status.connect(lambda m: self.scan_log.append_line(f"// {m}", "muted"))
        self._scan_worker.error.connect(lambda m: self.scan_log.append_line(f"! {m}", "error"))
        self._scan_worker.progress.connect(lambda d, n: self.scan_progress.setValue(d))
        self._scan_worker.done.connect(self._on_scan_done)
        self._scan_worker.start()

    def _on_scan_stop(self):
        if self._scan_worker:
            self._scan_worker.stop()   # cooperative — cancels the in-flight topic too

    def _on_scan_topic_started(self, topic: str, idx: int, total: int):
        self.scan_log.append_line(f"[{idx}/{total}] scanning '{topic}' …", "muted")

    def _on_scan_topic_done(self, topic: str, n_received: int):
        if n_received > 0:
            self.scan_log.append_line(
                f"  └─ {n_received} message(s) received — [!] ACL bypass, see FINDINGS", "crit"
            )
        else:
            self.scan_log.append_line("  └─ no messages received", "info")

    def _on_scan_done(self, scanned: int, yielded: int):
        self.btn_scan_all.setEnabled(True); self.btn_scan_stop.setEnabled(False)
        self.scan_log.append_line(
            f"// Scan All complete — {scanned} topic(s) scanned, {yielded} yielded data", "ok"
        )
...
class KafkaOffsetWorker(QThread):
    ...
    def run(self):
        ...
        try:
            # Partition discovery uses a disposable probe group.id, never the
            # real target group — merely instantiating a Consumer under the
            # target's group.id can itself register/perturb that group's
            # membership before the actual reset below even runs. The reset
            # itself targets self.group directly via AdminClient, which needs
            # no group.id of its own.
            probe_conf = {**base_conf, "group.id": "kafkapt-offset-probe",
                          "enable.auto.commit": False}
            c    = Consumer(probe_conf)
...
class AttackPanel(QGroupBox):
    ...
    def _prune_workers(self):
        """Drop finished QThread workers so self._workers doesn't grow for
        the life of the session — every attack handler calls this before
    ...
    def _on_inject(self):
        ...
        reply = QMessageBox.warning(
            self, "Header Injection — Confirm",
            f"Send attacker payload headers to '{topic}' on {broker}:\n"
            f"{[n for n, _ in headers]}\n\n"
            "This delivers a live injection payload (e.g. Log4Shell, SSTI, SSRF) to\n"
            "downstream consumers of this topic.\n\n"
            "Proceed only with explicit authorisation.",
            QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel,
        )
        if reply != QMessageBox.StandardButton.Ok: return
        ...
        self._prune_workers(); self._workers.append(worker); worker.start()
    ...
    def _on_acl_write(self):
        ...
        if not broker or not topic:
            self.atk_log.append_line("! broker and topic required", "error"); return
        reply = QMessageBox.warning(
            self, "ACL Write Test — Confirm",
            f"Attempt to write a marker message to '{topic}' on {broker}.\n\n"
            "If this topic is meant to be off-limits, a successful write means\n"
            "your credentials can inject data into it — a real, live write.\n\n"
            "Proceed only with explicit authorisation.",
            QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel,
        )
        if reply != QMessageBox.StandardButton.Ok: return
...
class FindingsPanel(QGroupBox):
    ...
    def _build(self):
        ...
        log_path = FindingStore.get().get_log_path()
        if log_path:
            self.lbl_autolog = QLabel(f"auto-saving every finding → {log_path}")
            self.lbl_autolog.setObjectName("lbl_status_inf")
        else:
            self.lbl_autolog = QLabel(
                "⚠ auto-save log could not be opened — findings exist only in memory "
                "until you click EXPORT MARKDOWN"
            )
            self.lbl_autolog.setObjectName("lbl_status_warn")
        self.lbl_autolog.setWordWrap(True)
        root.addWidget(self.lbl_autolog)
...
# ─────────────────────────────────────────────────────────
#  Background worker — Kafka Connect REST API  [Sprint 3]
# ─────────────────────────────────────────────────────────

_JDBC_CRED_RE = re.compile(
    r'jdbc:[a-z0-9+.\-]+://[^:@/\s]+:[^@\s]+@', re.IGNORECASE
)


class KafkaConnectWorker(QThread):
    """
    Enumerates Kafka Connect REST API (default port 8083):
      GET /connectors              — list all connector names
      GET /connectors/{n}/config  — full config (may contain credentials)
      GET /connectors/{n}/status  — running state
      GET /connector-plugins       — installed plugin classes

    Auto-finding rules:
      • Unauthenticated access          → MEDIUM
      • Credential key in config value  → HIGH
      • JDBC URL with embedded creds    → HIGH
    """
    connectors_found = pyqtSignal(list)            # list[str]
    connector_detail = pyqtSignal(str, dict, str)  # (name, config, state)
    plugins_found    = pyqtSignal(list)            # list[dict]
    status           = pyqtSignal(str)
    error            = pyqtSignal(str)
    done             = pyqtSignal()

    def __init__(self, server_url: str, phases: list[str],
                 basic_user: str = "", basic_pass: str = "",
    ...
    def _session(self) -> requests.Session:
        ...
        if self.basic_user:
            sess.auth = (self.basic_user, self.basic_pass)
        return sess

    def _scan_config(self, name: str, cfg: dict):
        hits = []
        for key, value in cfg.items():
            kl = key.lower()
            if any(kw in kl for kw in CRED_KEYWORDS):
                hits.append(f"  key='{key}'  value='{str(value)[:80]}'")
            if isinstance(value, str) and _JDBC_CRED_RE.search(value):
                hits.append(f"  JDBC URL with embedded credentials: '{value[:120]}'")
        if hits:
            FindingStore.get().add(Finding(
                severity="HIGH",
                title=f"Kafka Connect: credentials in connector '{name}' config",
                description=(
                    f"Connector '{name}' config contains values that appear to be "
                    "credentials/secrets, exposed to any caller with REST API access."
                ),
                evidence="\n".join(hits),
                phase="data",
            ))
    ...
    def run(self):
        ...
        # Phase 1 — list connectors
        if "list" in self.phases and not self._stop.is_set():
            self.status.emit("listing connectors …")
            try:
                r = sess.get(f"{self.server_url}/connectors", timeout=10)
                if r.status_code == 200:
                    names = r.json()
                    self.connectors_found.emit(names)
                    self.status.emit(f"{len(names)} connector(s) found")
                    if not self.basic_user:
                        FindingStore.get().add(Finding(
                            severity="MEDIUM",
                            title="Kafka Connect REST API accessible without authentication",
                            description=(
                                f"GET {self.server_url}/connectors → HTTP 200 without credentials. "
                                "Unauthenticated access exposes connector names, types, and configs."
                            ),
                            evidence=f"GET {self.server_url}/connectors → HTTP 200\n{names}",
                            phase="recon",
                        ))
                elif r.status_code == 401:
                    self.status.emit("HTTP 401 — authentication required")
                else:
                    self.error.emit(f"HTTP {r.status_code}: {r.text[:120]}")
                    self.done.emit(); return
            except Exception as exc:
                self.error.emit(f"list error: {exc}"); self.done.emit(); return

        # Phase 2 — fetch configs
        if "configs" in self.phases and not self._stop.is_set():
            try:
                r = sess.get(f"{self.server_url}/connectors", timeout=10)
                names = r.json() if r.ok else []
            except Exception:
                names = []
            for name in names:
                if self._stop.is_set(): break
                self.status.emit(f"fetching config: '{name}' …")
                try:
                    rc = sess.get(f"{self.server_url}/connectors/{name}/config",  timeout=10)
                    rs = sess.get(f"{self.server_url}/connectors/{name}/status",  timeout=10)
                    cfg   = rc.json() if rc.ok else {}
                    state = (rs.json().get("connector", {}).get("state", "")
                             if rs.ok else "")
                    self.connector_detail.emit(name, cfg, state)
                    self._scan_config(name, cfg)
                except Exception as exc:
                    self.error.emit(f"config '{name}': {exc}")

        # Phase 3 — list plugins
        if "plugins" in self.phases and not self._stop.is_set():
            self.status.emit("listing plugins …")
            try:
                r = sess.get(f"{self.server_url}/connector-plugins", timeout=10)
                if r.ok:
                    self.plugins_found.emit(r.json())
            except Exception as exc:
                self.error.emit(f"plugins error: {exc}")

        self.done.emit()


# ─────────────────────────────────────────────────────────
#  ConnectPanel — Kafka Connect REST Explorer  [Sprint 3]
# ─────────────────────────────────────────────────────────

class ConnectPanel(QGroupBox):
    def __init__(self):
        super().__init__("[ KAFKA CONNECT — REST API EXPLORER ]")
        self._worker: KafkaConnectWorker | None = None
    ...
    def _build(self):
        ...
        # Connection row
        conn_row = QHBoxLayout()
        lbl_srv = QLabel("Connect Server"); lbl_srv.setObjectName("lbl_key")
        self.inp_server = QLineEdit()
        self.inp_server.setPlaceholderText("http://kafka-connect:8083")
        self.inp_server.setToolTip("Kafka Connect REST API address (default port 8083)")
        conn_row.addWidget(lbl_srv); conn_row.addWidget(self.inp_server, 1)
        root.addLayout(conn_row)

        # Auth row
        auth_row = QHBoxLayout()
        self.chk_auth = QCheckBox("Basic Auth")
        self.inp_auth_user = QLineEdit(); self.inp_auth_user.setPlaceholderText("username")
        self.inp_auth_user.setFixedWidth(140); self.inp_auth_user.setEnabled(False)
        self.inp_auth_pass = QLineEdit(); self.inp_auth_pass.setPlaceholderText("password")
        self.inp_auth_pass.setEchoMode(QLineEdit.EchoMode.Password)
        self.inp_auth_pass.setFixedWidth(140); self.inp_auth_pass.setEnabled(False)
        self.chk_verify_ssl = QCheckBox("Verify SSL"); self.chk_verify_ssl.setChecked(True)
        self.chk_auth.toggled.connect(self.inp_auth_user.setEnabled)
        self.chk_auth.toggled.connect(self.inp_auth_pass.setEnabled)
        auth_row.addWidget(self.chk_auth)
        auth_row.addWidget(self.inp_auth_user); auth_row.addWidget(self.inp_auth_pass)
        auth_row.addSpacing(16); auth_row.addWidget(self.chk_verify_ssl)
        auth_row.addStretch()
        root.addLayout(auth_row)

        # Buttons
        btn_row = QHBoxLayout()
        self.btn_list    = QPushButton("LIST CONNECTORS"); self.btn_list.setObjectName("btn_accent")
        self.btn_scan    = QPushButton("FULL SCAN");       self.btn_scan.setObjectName("btn_warn")
        self.btn_plugins = QPushButton("SHOW PLUGINS")
        self.btn_stop    = QPushButton("STOP");            self.btn_stop.setObjectName("btn_danger")
        self.btn_stop.setEnabled(False)
        btn_row.addWidget(self.btn_list); btn_row.addWidget(self.btn_scan)
        btn_row.addWidget(self.btn_plugins); btn_row.addSpacing(12)
        btn_row.addWidget(self.btn_stop); btn_row.addStretch()
        root.addLayout(btn_row)

        # Splitter: tree (left) + config detail (right)
        splitter = QSplitter(Qt.Orientation.Horizontal); splitter.setHandleWidth(4)

        left  = QWidget(); left_v = QVBoxLayout(left); left_v.setContentsMargins(0, 0, 4, 0)
        lbl_l = QLabel("Connectors / Plugins"); lbl_l.setObjectName("lbl_key")
        self.tree = QTreeWidget(); self.tree.setHeaderLabels(["Name", "State"])
        self.tree.setColumnWidth(0, 220); self.tree.setAlternatingRowColors(True)
        left_v.addWidget(lbl_l); left_v.addWidget(self.tree, 1)
        splitter.addWidget(left)

        right  = QWidget(); right_v = QVBoxLayout(right); right_v.setContentsMargins(4, 0, 0, 0)
        lbl_r  = QLabel("Configuration"); lbl_r.setObjectName("lbl_key")
        self.cfg_view = QTextEdit(); self.cfg_view.setReadOnly(True)
        self.cfg_view.setFont(QFont("Courier New", 11))
        self.cfg_view.setPlaceholderText(
            "// select a connector to view full config\n"
            "// credential keys are annotated automatically"
        )
        right_v.addWidget(lbl_r); right_v.addWidget(self.cfg_view, 1)
        splitter.addWidget(right)
        splitter.setSizes([260, 500])
        root.addWidget(splitter, 1)

        # Status log
        self.connect_log = LogPane("// Kafka Connect output")
        self.connect_log.setMaximumHeight(90)
        root.addWidget(self.connect_log)

        self.btn_list.clicked.connect(lambda: self._run(["list"]))
        self.btn_scan.clicked.connect(lambda: self._run(["list", "configs"]))
        self.btn_plugins.clicked.connect(lambda: self._run(["plugins"]))
        self.btn_stop.clicked.connect(self._on_stop)
    ...
    def _run(self, phases: list[str]):
        ...
        if not server:
            self.connect_log.append_line("! Connect Server URL required", "error"); return
        if self._worker and self._worker.isRunning(): return
        self.connect_log.append_line(f"// {phases}  →  {server}", "muted")
        for b in (self.btn_list, self.btn_scan, self.btn_plugins): b.setEnabled(False)
        self.btn_stop.setEnabled(True)
        self._worker = KafkaConnectWorker(
            server, phases,
            basic_user=self.inp_auth_user.text().strip() if self.chk_auth.isChecked() else "",
            basic_pass=self.inp_auth_pass.text()         if self.chk_auth.isChecked() else "",
            verify_ssl=self.chk_verify_ssl.isChecked(),
        )
        self._worker.connectors_found.connect(self._on_connectors)
        self._worker.connector_detail.connect(self._on_detail)
        self._worker.plugins_found.connect(self._on_plugins)
        self._worker.status.connect(lambda m: self.connect_log.append_line(f"// {m}", "muted"))
        self._worker.error.connect(lambda m: self.connect_log.append_line(f"! {m}", "error"))
        self._worker.done.connect(self._on_done)
        self._worker.start()
    ...
    def _on_connectors(self, names: list):
        self.tree.clear(); self._connectors.clear()
        parent = QTreeWidgetItem(self.tree, ["CONNECTORS", f"({len(names)})"])
        parent.setExpanded(True)
        for n in sorted(names):
            QTreeWidgetItem(parent, [n, "—"])
        self.connect_log.append_line(f"// {len(names)} connector(s): {names}", "ok")

    def _on_detail(self, name: str, cfg: dict, state: str):
        self._connectors[name] = {"config": cfg, "state": state}
        ...
        for ti in range(root.childCount()):
            ...
            for ci in range(p.childCount()):
                ...
                if child.text(0) == name:
                    ...
                    color = ("#4caf50" if state.upper() == "RUNNING"
                             else "#cf4444" if state.upper() in ("FAILED", "UNASSIGNED")
                             else "#b8860b")
                    child.setForeground(1, QColor(color))
                    break

    def _on_plugins(self, plugins: list):
        parent = QTreeWidgetItem(self.tree, ["PLUGINS", f"({len(plugins)})"])
        parent.setExpanded(True)
        for p in plugins:
            cls  = p.get("class", "?").split(".")[-1]
            ptype = p.get("type", ""); ver = p.get("version", "")
            QTreeWidgetItem(parent, [cls, f"{ptype} {ver}".strip()])
        self.connect_log.append_line(f"// {len(plugins)} plugin(s)", "ok")

    def _on_select(self, current, _prev):
        if current is None: return
        name = current.text(0)
        if name not in self._connectors: return
        cfg   = self._connectors[name]["config"]
        state = self._connectors[name]["state"]
        lines = []
        for line in json.dumps(cfg, indent=2, ensure_ascii=False).splitlines():
            ann = ""
            ll  = line.lower()
            if any(f'"{kw}"' in ll or f"'{kw}'" in ll for kw in CRED_KEYWORDS):
                ann = "  ← [CREDENTIAL KEY]"
            elif _JDBC_CRED_RE.search(line):
                ann = "  ← [EMBEDDED CRED IN URL]"
            lines.append(line + ann)
        self.cfg_view.setPlainText(
            f"// connector: {name}  state: {state}\n\n" + "\n".join(lines)
        )

    def _on_done(self):
        for b in (self.btn_list, self.btn_scan, self.btn_plugins): b.setEnabled(True)
        self.btn_stop.setEnabled(False)
        self.connect_log.append_line("// done", "ok")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle(f"KafkaPT v{APP_VERSION}  //  Kafka Pentest Toolkit")

        # ── v2.7: auto-persist findings from the moment the app opens ──
        # A crash or an accidental window close should not lose evidence
        # collected before the operator remembers to click EXPORT MARKDOWN.
        # Started *before* _build() so FindingsPanel can show the path
        # (or the failure warning) as soon as it's constructed.
        log_dir  = os.path.join(os.path.dirname(os.path.abspath(__file__)), "kafkapt_sessions")
        ts       = datetime.datetime.utcnow().strftime("%Y%m%d-%H%M%S")
        log_path = os.path.join(log_dir, f"session-{ts}.jsonl")
        self._log_ok = FindingStore.get().start_log(log_path)
        ...
        if self._log_ok:
            self.status_bar.showMessage(
                f"KafkaPT v{APP_VERSION} ready — auto-saving findings to {log_path}  "
                "— set a Target above to label this engagement"
            )
        else:
            self.status_bar.showMessage(
                f"KafkaPT v{APP_VERSION} ready — WARNING: could not open evidence log at "
                f"{log_path} — findings will only exist in memory until manually exported"
            )
    ...
    def _build(self):
        ...
        lbl_target.setObjectName("lbl_key")
        lbl_target.setFixedWidth(46)
        self.inp_target = QLineEdit()
        self.inp_target.setPlaceholderText("engagement / client name — e.g. AcmeCorp-2026Q1")
        self.inp_target.setFixedWidth(220)
        self.inp_target.setToolTip(
            "Stamped on every finding recorded from now on, and shown in the\n"
            "window title. Set this before starting a new engagement so findings\n"
            "from two different targets in the same running session can't mix."
        )
        self.inp_target.textChanged.connect(self._on_target_changed)
        ...
        header_row.addWidget(lbl_target)
        header_row.addWidget(self.inp_target)
        ...
        self.lbl_sub = QLabel(f"Kafka Pentest Toolkit  /  v{APP_VERSION}")
        ...
        # Config panels (built first — ops panels reference them)
        ...
        # ── Single unified QTabWidget ────────────────────────
        #
        #  ⚙ CONFIG | RECON | ATTACK | CONNECT | READ | SEND | RANDOMIZE | FINDINGS
        #
        main_tabs = QTabWidget()

        # TAB 1 — CONFIGURATION (wraps the 4 config sub-tabs)
        # No max-height constraint here — full window height available.
        config_widget = QWidget()
        config_vbox   = QVBoxLayout(config_widget)
        config_vbox.setContentsMargins(6, 6, 6, 6)
        config_vbox.setSpacing(0)
        cfg_sub = QTabWidget()
        cfg_sub.addTab(self.schema_panel, "SCHEMA")
        cfg_sub.addTab(self.cert_panel,   "CERTIFICATES")
        cfg_sub.addTab(self.enc_panel,    "ENCRYPTION")
        cfg_sub.addTab(self.proxy_panel,  "PROXY / BURP")
        config_vbox.addWidget(cfg_sub)
        main_tabs.addTab(config_widget,       "⚙  CONFIG")

        # TABs 2–8 — Operations
        main_tabs.addTab(self.recon_panel,    "RECON")
        main_tabs.addTab(self.attack_panel,   "ATTACK")
        main_tabs.addTab(self.connect_panel,  "CONNECT")
        main_tabs.addTab(self.reader_panel,   "READ")
        main_tabs.addTab(self.writer_panel,   "SEND")
        main_tabs.addTab(self.rand_panel,     "RANDOMIZE")
        main_tabs.addTab(self.findings_panel, "FINDINGS")

        root.addWidget(main_tabs, 1)

    def _on_target_changed(self, text: str):
        """v2.7 — stamp the engagement/target name on every finding from
        this point on, and reflect it in the window title so it's obvious
        at a glance which engagement a running session is pointed at."""
        FindingStore.get().set_target(text)
        name = text.strip()
        if name:
            self.setWindowTitle(f"KafkaPT v{APP_VERSION}  //  {name}")
        else:
            self.setWindowTitle(f"KafkaPT v{APP_VERSION}  //  Kafka Pentest Toolkit")
